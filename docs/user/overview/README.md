...
MCP servers, single agents, and orchestrates multi-server stacks via `smithy compose`.
...
- **One CLI, the whole stack** — run a single MCP server, a single
  agent, or orchestrate many of them with `smithy compose`.
...
1. You describe your servers and agents in YAML
   (`.mcpsmithy.yaml`, `.agentsmithy.yaml`, or a `smithy-compose.yaml`
...
2. You run a single component with `smithy mcp serve` or
   ...
   `smithy compose up` — a built-in supervisor that spawns, monitors
